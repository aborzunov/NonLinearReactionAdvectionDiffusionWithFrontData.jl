...
# Регрессивный тест
# На точных данных, градиент должен быть мал
@testset "Градиент на динамической сетки, точные данные   " begin
    ...
    @test_broken norm(grad) < 0.05             # L2 норма
    @test_broken norm(grad, Inf) < 0.06        # Максимальный элемент в векторе max(abs.(v))
    @test_broken 1 < 0
