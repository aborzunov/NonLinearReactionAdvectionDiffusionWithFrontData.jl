...
@testset "Градиент на точных данных и динамической сетке   " begin
    ...
    @test nnz(hmap(0.0001)) > M * 0.05           # Много ненулевых элементов,
                                                # значит итерационный процесс продолжится и уйдет от решения
    @test nnz(hmap(0.0000001)) == 0              # 0, слишком узкая дельта — Не рекомендовано
    @test 0 < nnz(hmap(0.000003)) <= M * 0.01   # Немного ненулевых — Должно быть норм!
